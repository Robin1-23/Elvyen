...
class VisitorManager:
    ...
    async def broadcast_count(self):
        ...
        message = json.dumps({"visitors": count})
        dead = set()
...
async def visitor_websocket(websocket: WebSocket):
    await visitor_manager.connect(websocket)
    try:
        while True:
            await websocket.receive_text()  # keep alive
    except WebSocketDisconnect:
        visitor_manager.disconnect(websocket)
        await visitor_manager.broadcast_count()

@api_router.get("/visitors")
async def get_visitor_count():
    return {"visitors": len(visitor_manager.active_connections)}
