...
async def schedule_meeting(request: MeetingScheduleRequest):
    ...
    """
    # FIX: Define meeting_data outside try so it's accessible in except block
    meeting_data = None
    meeting_stored = False
    ...
    try:
        ...
        # FIX: Use meeting_stored flag instead of checking meeting_data in exception string
        if meeting_stored:
            return {
                "status": "success",
